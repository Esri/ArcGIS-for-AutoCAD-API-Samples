...
This sample shows how to generate a Polyline2d feature with m-values from an Autodesk Civil 3D horizontal alignment. The m-values of the resultant feature capture the alignment's stationing.

## Description
This example creates a Polyline2d entity from a Civil 3D alignment then writes the equivalent station value to each polyline vertex as an m-value.
...
_The **AFA_Samples_AlignmentToMPolyline** sample command uses the following ArcGIS for AutoCAD .NET API methods:_
